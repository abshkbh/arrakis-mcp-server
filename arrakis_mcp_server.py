#!/usr/bin/env python3
"""
Arrakis MCP Server - Model Context Protocol server for Arrakis VM sandboxes.

This module implements an MCP server that exposes Arrakis VM sandbox functionality
to LLMs through the Model Context Protocol (MCP).
"""

import os
import sys
import argparse
import json
import logging
from typing import List, Dict, Any, Optional
from contextlib import asynccontextmanager
from dataclasses import dataclass

# Import MCP SDK
from mcp.server.fastmcp import FastMCP, Context

# Import py_arrakis functionality
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
from py_arrakis.client import APIClient
from py_arrakis.sandbox import Sandbox
from py_arrakis.sandbox_manager import SandboxManager

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    handlers=[logging.StreamHandler()],
)
logger = logging.getLogger("arrakis-mcp")

# Default Arrakis server URL
DEFAULT_ARRAKIS_URL = "http://localhost:8080"


@dataclass
class ArrakisContext:
    """Context object for Arrakis integration."""
    sandbox_manager: SandboxManager
    active_sandbox: Optional[Sandbox] = None


@asynccontextmanager
async def arrakis_lifespan(server: FastMCP):
    """
    Set up and tear down the Arrakis context for the MCP server.
    
    This manages the lifecycle of the connection to the Arrakis server.
    """
    # Initialize Arrakis client and manager
    arrakis_url = os.environ.get("ARRAKIS_SERVER_URL", DEFAULT_ARRAKIS_URL)
    logger.info(f"Connecting to Arrakis server at {arrakis_url}")
    
    sandbox_manager = SandboxManager(arrakis_url)
    context = ArrakisContext(sandbox_manager=sandbox_manager)
    
    try:
        # Yield the context to the server
        yield context
    finally:
        # Clean up resources if needed
        if context.active_sandbox:
            logger.info(f"Destroying active sandbox {context.active_sandbox.name}")
            try:
                context.active_sandbox.destroy()
            except Exception as e:
                logger.error(f"Error destroying sandbox: {e}")


def create_mcp_server() -> FastMCP:
    """Create and configure the MCP server."""
    # Create the MCP server with a name and dependencies
    mcp = FastMCP(
        "Arrakis Sandbox Manager",
        dependencies=["requests"],
        lifespan=arrakis_lifespan
    )
    
    # Define MCP resources
    @mcp.resource("arrakis://vms")
    def get_vms(ctx: Context) -> str:
        """List all available VMs."""
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            vms = arrakis_ctx.sandbox_manager.list_all()
            vm_info = [vm.info() for vm in vms]
            return json.dumps(vm_info, indent=2)
        except Exception as e:
            return f"Error listing VMs: {str(e)}"
    
    @mcp.resource("arrakis://vm/{vm_name}")
    def get_vm_info(ctx: Context, vm_name: str) -> str:
        """Get information about a specific VM."""
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            sandbox = Sandbox(arrakis_ctx.sandbox_manager._api, vm_name)
            return json.dumps(sandbox.info(), indent=2)
        except Exception as e:
            return f"Error retrieving VM information: {str(e)}"
    
    # Define MCP tools
    @mcp.tool()
    def start_sandbox(ctx: Context, name: str) -> str:
        """
        Start a new sandbox VM.
        
        Args:
            name: Name to give to the new VM.
            
        Returns:
            Information about the created VM.
        """
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            sandbox = arrakis_ctx.sandbox_manager.start_sandbox(name)
            arrakis_ctx.active_sandbox = sandbox
            return json.dumps(sandbox.info(), indent=2)
        except Exception as e:
            return f"Error starting sandbox: {str(e)}"
    
    @mcp.tool()
    def restore_snapshot(ctx: Context, vm_name: str, snapshot_id: str) -> str:
        """
        Restore a VM from a snapshot.
        
        Args:
            vm_name: Name to give to the restored VM.
            snapshot_id: ID of the snapshot to restore from.
            
        Returns:
            Information about the restored VM.
        """
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            sandbox = arrakis_ctx.sandbox_manager.restore(vm_name, snapshot_id)
            arrakis_ctx.active_sandbox = sandbox
            return json.dumps(sandbox.info(), indent=2)
        except Exception as e:
            return f"Error restoring snapshot: {str(e)}"
    
    @mcp.tool()
    def create_snapshot(ctx: Context, vm_name: str, snapshot_id: str = "") -> str:
        """
        Create a snapshot of a VM.
        
        Args:
            vm_name: Name of the VM to snapshot.
            snapshot_id: Optional ID for the snapshot. If not provided, one will be generated.
            
        Returns:
            ID of the created snapshot.
        """
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            sandbox = Sandbox(arrakis_ctx.sandbox_manager._api, vm_name)
            result = sandbox.snapshot(snapshot_id)
            return f"Created snapshot with ID: {result}"
        except Exception as e:
            return f"Error creating snapshot: {str(e)}"
    
    @mcp.tool()
    def run_command(ctx: Context, vm_name: str, cmd: str) -> str:
        """
        Run a command in a VM.
        
        Args:
            vm_name: Name of the VM to run the command in.
            cmd: Command to execute.
            
        Returns:
            Command output and/or error.
        """
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            sandbox = Sandbox(arrakis_ctx.sandbox_manager._api, vm_name)
            result = sandbox.run_cmd(cmd)
            output = result.get("output", "")
            error = result.get("error", "")
            
            if error:
                return f"Command output:\n{output}\n\nError:\n{error}"
            return f"Command output:\n{output}"
        except Exception as e:
            return f"Error running command: {str(e)}"
    
    @mcp.tool()
    def upload_file(ctx: Context, vm_name: str, path: str, content: str) -> str:
        """
        Upload a file to a VM.
        
        Args:
            vm_name: Name of the VM to upload to.
            path: Destination path in the VM.
            content: Content of the file.
            
        Returns:
            Success/failure message.
        """
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            sandbox = Sandbox(arrakis_ctx.sandbox_manager._api, vm_name)
            sandbox.upload_files([{"path": path, "content": content}])
            return f"Successfully uploaded file to {path}"
        except Exception as e:
            return f"Error uploading file: {str(e)}"
    
    @mcp.tool()
    def download_file(ctx: Context, vm_name: str, path: str) -> str:
        """
        Download a file from a VM.
        
        Args:
            vm_name: Name of the VM to download from.
            path: Path of the file to download.
            
        Returns:
            Content of the file or error message.
        """
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            sandbox = Sandbox(arrakis_ctx.sandbox_manager._api, vm_name)
            result = sandbox.download_files([path])
            
            if not result:
                return f"No file found at path: {path}"
                
            file_data = result[0]
            if "error" in file_data and file_data["error"]:
                return f"Error downloading file: {file_data['error']}"
                
            return f"File content:\n{file_data.get('content', '')}"
        except Exception as e:
            return f"Error downloading file: {str(e)}"
    
    @mcp.tool()
    def destroy_vm(ctx: Context, vm_name: str) -> str:
        """
        Destroy a VM.
        
        Args:
            vm_name: Name of the VM to destroy.
            
        Returns:
            Success/failure message.
        """
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            sandbox = Sandbox(arrakis_ctx.sandbox_manager._api, vm_name)
            sandbox.destroy()
            
            # If we destroyed the active sandbox, clear the reference
            if arrakis_ctx.active_sandbox and arrakis_ctx.active_sandbox.name == vm_name:
                arrakis_ctx.active_sandbox = None
                
            return f"Successfully destroyed VM {vm_name}"
        except Exception as e:
            return f"Error destroying VM: {str(e)}"
    
    @mcp.tool()
    def destroy_all_vms(ctx: Context) -> str:
        """
        Destroy all VMs.
        
        Returns:
            Success/failure message.
        """
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            arrakis_ctx.sandbox_manager.destroy_all()
            arrakis_ctx.active_sandbox = None
            return "Successfully destroyed all VMs"
        except Exception as e:
            return f"Error destroying VMs: {str(e)}"
    
    @mcp.tool()
    def update_vm_state(ctx: Context, vm_name: str, status: str) -> str:
        """
        Update the state of a VM.
        
        Args:
            vm_name: Name of the VM to update.
            status: New state for the VM. Must be either 'stopped' or 'paused'.
            
        Returns:
            Success/failure message.
        """
        arrakis_ctx = ctx.request_context.lifespan_context
        try:
            sandbox = Sandbox(arrakis_ctx.sandbox_manager._api, vm_name)
            sandbox.update_state(status)
            return f"Successfully updated VM {vm_name} state to {status}"
        except Exception as e:
            return f"Error updating VM state: {str(e)}"
    
    # Return the configured MCP server
    return mcp


def main():
    """Run the MCP server."""
    parser = argparse.ArgumentParser(description="Arrakis MCP Server")
    parser.add_argument(
        "--arrakis-url", 
        default=DEFAULT_ARRAKIS_URL,
        help=f"URL of the Arrakis server (default: {DEFAULT_ARRAKIS_URL})"
    )
    args = parser.parse_args()
    
    # Set environment variable for the Arrakis server URL
    os.environ["ARRAKIS_SERVER_URL"] = args.arrakis_url
    
    # Create and run the MCP server
    mcp = create_mcp_server()
    
    # Run the server (this will block until the user closes the connection)
    try:
        logger.info("Starting Arrakis MCP Server...")
        # Uses a STDIO transport by default when no arguments are provided
        mcp.run()
    except KeyboardInterrupt:
        logger.info("Server shutting down...")
    except Exception as e:
        logger.error(f"Error running server: {e}")
        return 1
    
    return 0


if __name__ == "__main__":
    sys.exit(main())